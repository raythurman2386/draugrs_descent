...
def find_closest_enemy(player_pos, enemies):
    ...
    closest = min(enemies, key=lambda e: math.hypot(
        e.rect.centerx - player_pos[0], e.rect.centery - player_pos[1]))
    
    logger.debug(f"Found closest enemy at {closest.rect.center}, distance: {math.hypot(closest.rect.centerx - player_pos[0], closest.rect.centery - player_pos[1]):.2f}")
    return closest
