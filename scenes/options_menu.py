...
class OptionsMenuScene(Scene):
    ...
    def load_ui_assets(self):
        ...
        self.slider_back = game_asset_manager.get_svg_ui_element(
            "slide_horizontal_color", slider_theme, scale=(slider_width, slider_height)
        )

        # Load slider knob (using SVG) - specify scale dimensions
        self.slider_knob = game_asset_manager.get_svg_ui_element(
            "slide_hangle", knob_theme, scale=(knob_size, knob_size)
        )
    ...
    def render(self):
        ...
        section_color = config.get(
            "ui", "options_menu", "sections", "audio", "color", default="white"
        )
        section = self.option_font.render(section_title, True, config.get_color(section_color))
        section_rect = section.get_rect(center=(self.screen_width // 2, self.panel_rect.top + 100))
        ...
        instructions = self.instruction_font.render(
            instruction_text, True, config.get_color("white")
        )
