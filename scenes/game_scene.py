...
from constants import (
    ...
    FRAME_RATE, ENEMY_SPAWN_INTERVAL, ENEMY_SPAWN_EDGES,
...
class GameScene(Scene):
    ...
    def spawn_enemy(self):
        ...
        if current_time - self.last_spawn_time > ENEMY_SPAWN_INTERVAL:
            ...
            if edge == 0:  # Top
                position = (random.randint(0, SCREEN_WIDTH), ENEMY_SPAWN_EDGES['TOP'][1])
            elif edge == 1:  # Bottom
                position = (random.randint(0, SCREEN_WIDTH), ENEMY_SPAWN_EDGES['BOTTOM'][1])
            elif edge == 2:  # Left
                position = (ENEMY_SPAWN_EDGES['LEFT'][0], random.randint(0, SCREEN_HEIGHT))
            else:  # Right
                position = (ENEMY_SPAWN_EDGES['RIGHT'][0], random.randint(0, SCREEN_HEIGHT))
    ...
    def update(self):
        ...
        if not self.paused:
            ...
            if self.player.current_health <= 0:
                logger.info(f"Game over. Enemies killed: {self.enemies_killed}, Time survived: {self.time_survived}s")
    ...
    def render(self):
        ...
        pygame.draw.rect(self.screen, (255, 0, 0), (10, 10, HEALTH_BAR_WIDTH, HEALTH_BAR_HEIGHT))  # Background
        pygame.draw.rect(self.screen, (0, 255, 0), (10, 10, HEALTH_BAR_WIDTH * health_ratio, HEALTH_BAR_HEIGHT))  # Health
